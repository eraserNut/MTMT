...
from dataloaders.ISIC import ISIC
import dataloaders.joint_transforms_edge as joint_transforms
from utils.util import AverageMeter, TwoStreamBatchSampler, relabel_dataset
parser = argparse.ArgumentParser()
# parser.add_argument('--root_path', type=str, default='/media/data/chenzhihao/datasets/ISIC-2017', help='Name of Experiment')
...
parser.add_argument('--label_consistency', type=int,  default=1, help='consistency loss for label data')
parser.add_argument('--max_iterations', type=int,  default=7000, help='maximum epoch number to train')
...
# snapshot_path = "../model_SBU_EGNet/baselineC64_DSS_unlabelcons/"+'edge'+str(args.edge)+'lr'+str(args.base_lr)+'consistency'+str(args.consistency)+'repeat'+str(args.repeat)+'/'
snapshot_path = "../models_ISIC2018/MTMT_baseline_v2"
tmp_path = 'tmp_see'
...
if __name__ == "__main__":
    ...
    img_transform = transforms.Compose([
        transforms.ToTensor(),
        # transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) # suitable for SBU
        transforms.Normalize([0.723, 0.616, 0.569], [0.169, 0.177, 0.197])  # suitable for ISIC2017
    ])
    target_transform = transforms.ToTensor()
    to_pil = transforms.ToPILImage()


    db_train = ISIC(root=train_data_path, joint_transform=joint_transform, transform=img_transform, target_transform=target_transform, mod='union', edge=True)# set edge=True & mod='union' in MTMT
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            up_edge, up_shadow, up_shadow_final = model(image_batch)
            with torch.no_grad():
                up_edge_ema, up_shadow_ema, up_shadow_final_ema = ema_model(ema_inputs)
            ...
            for (ix, ix_ema) in zip(up_edge, up_edge_ema):
                ...
                if args.label_consistency:
                    edge_con_loss.append(consistency_criterion(ix, ix_ema))
                else:
            ...
            for (ix, ix_ema) in zip(up_shadow, up_shadow_ema):
                ...
                if args.label_consistency:
                    shadow_con_loss1.append(consistency_criterion(ix, ix_ema))
                else:
            ...
            for (ix, ix_ema) in zip(up_shadow_final, up_shadow_final_ema):
                ...
                if args.label_consistency:
                    shadow_con_loss2.append(consistency_criterion(ix, ix_ema))
                else:
            ...
            supervised_loss = shadow_loss + edge_loss * args.edge

            consistency_weight = get_current_consistency_weight(epoch_num)
            consistency_loss = consistency_weight * (edge_con_loss + sum(shadow_con_loss1) + sum(shadow_con_loss2))
            ...
            shadow_con_loss2_record.update(shadow_con_loss2[-1].item(), batch_size)
            edge_con_loss_record.update(edge_con_loss.item(), batch_size)

            logging.info('iteration %d : shadow_f : %f5 , edge: %f5 , shadow_f_con: %f5  edge_con: %f5 loss_weight: %f5, lr: %f5' %
